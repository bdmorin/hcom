...
class LaunchScreen:
    ...
    _tool_options = tuple(
        t
        for t in ("claude", "gemini", "codex")
        if t in RELEASED_TOOLS and not (IS_WINDOWS and t in _PTY_ONLY_TOOLS)
    )
    ...
    def _get_claude_defaults(self) -> tuple[str, str, str, bool]:
        ...
        if LaunchScreen._claude_defaults_cache is None:
            ...
            spec = resolve_claude_args(
                None, claude_args_default if claude_args_default else None
            )
    ...
    def build(self, height: int, width: int) -> List[str]:
        ...
        if field_info:
            ...
            editor_rows = (
                editor_content_rows + 4
            )  # +4 for separator, header, blank line, separator
        ...
        # Tool field
        tool_selected = self.state.launch_field == LaunchField.TOOL
        tool_installed = self._is_tool_installed(self.state.launch_tool)
        install_indicator = (
            f"{FG_GREEN}✓{RESET}" if tool_installed else f"{FG_RED}✗{RESET}"
        )
        if tool_selected:
            selected_field_start_line = len(lines)
            line = (
                f"  {BG_CHARCOAL}{FG_WHITE}{BOLD}\u25b8 Tool:{RESET}{BG_CHARCOAL} "
                f"{FG_ORANGE}{self.state.launch_tool}{RESET}{BG_CHARCOAL} {install_indicator}{BG_CHARCOAL}  {FG_GRAY}\u2022 \u2190\u2192 cycle{RESET}"
            )
            lines.append(bg_ljust(line, width, BG_CHARCOAL))
        else:
            lines.append(
                f"  {FG_WHITE}Tool:{RESET} {FG_ORANGE}{self.state.launch_tool}{RESET} {install_indicator}"
            )
        ...
        if count_selected:
            ...
            lines.append(
                f"  {FG_WHITE}Count:{RESET} {FG_ORANGE}{self.state.launch_count}{RESET}"
            )
        ...
        claude_set = 0
        if tool == "claude":
            default_prompt, default_system, default_append, default_background = (
                self._get_claude_defaults()
            )
            ...
            claude_args_val = (
                self.state.config_edit.get("HCOM_CLAUDE_ARGS", "").strip().strip("'\"")
            )
            claude_args_default_normalized = (
                CONFIG_DEFAULTS.get("HCOM_CLAUDE_ARGS", "").strip().strip("'\"")
            )
            ...
            tool_prompt = (
                self.state.launch_gemini_prompt
                if tool == "gemini"
                else self.state.launch_codex_prompt
            )
            if tool_prompt:
                claude_set += 1
            # Count tool-specific system prompt
            if tool == "gemini" and self.state.launch_gemini_system_prompt:
                claude_set += 1
            elif tool == "codex" and self.state.launch_codex_system_prompt:
                claude_set += 1
            if tool in RELEASED_BACKGROUND and self.state.launch_background:
                claude_set += 1
        ...
        if claude_selected:
            selected_field_start_line = len(lines)
            claude_action = (
                "\u2190 collapse" if self.state.claude_expanded else "\u2192 expand"
            )
            ...
            lines.append(
                f"  {FG_CLAUDE_ORANGE}{BOLD}{expand_marker} {tool_title}{RESET}{FG_GRAY}{claude_count}{RESET}"
            )
        ...
        if not self.state.claude_expanded:
            ...
            if tool == "claude":
                if self.state.launch_background != default_background:
                    previews.append(
                        "headless: true"
                        if self.state.launch_background
                        else "headless: false"
                    )
                if self.state.launch_prompt != default_prompt:
                    prompt_str = (
                        str(self.state.launch_prompt)
                        if self.state.launch_prompt
                        else ""
                    )
                    if prompt_str:
                        has_ui_prompt = True
                ...
                if self.state.launch_system_prompt != default_system:
                    sys_str = (
                        str(self.state.launch_system_prompt)
                ...
                if self.state.launch_append_system_prompt != default_append:
                    append_str = (
                        str(self.state.launch_append_system_prompt)
                        if self.state.launch_append_system_prompt
                        else ""
                    )
                    append_preview = (
                        append_str[:20] + "..." if len(append_str) > 20 else append_str
                    )
                    previews.append(f'append: "{append_preview}"')
                if claude_args_val != claude_args_default_normalized:
                    args_str = str(claude_args_val) if claude_args_val else ""
                    args_preview = (
                        args_str[:25] + "..." if len(args_str) > 25 else args_str
                    )
                    previews.append(f'args: "{args_preview}"')
                # Check for interactive mode (no prompt)
                if not has_ui_prompt and not self.state.launch_prompt:
                    claude_args_str = self.state.config_edit.get("HCOM_CLAUDE_ARGS", "")
                    claude_spec = resolve_claude_args(
                        None, claude_args_str if claude_args_str else None
                    )
                ...
                if tool_prompt:
                    ...
                    prompt_preview = (
                        tool_prompt[:20] + "..."
                        if len(tool_prompt) > 20
                        else tool_prompt
                    )
                    previews.append(f'prompt: "{prompt_preview}"')
                # Tool-specific system prompt preview
                sys_prompt = (
                    self.state.launch_gemini_system_prompt
                    if tool == "gemini"
                    else self.state.launch_codex_system_prompt
                )
                if sys_prompt:
                    sys_preview = (
                        sys_prompt[:20] + "..." if len(sys_prompt) > 20 else sys_prompt
                    )
                    previews.append(f'system: "{sys_preview}"')
                if tool in RELEASED_BACKGROUND and self.state.launch_background:
                ...
                if not has_ui_prompt:
                    ...
                    if tool == "gemini":
                        gemini_spec = resolve_gemini_args(
                            None, args_str if args_str else None
                        )
                        has_args_prompt = bool(
                            gemini_spec.positional_tokens
                        ) or gemini_spec.has_flag(["-i", "--prompt-interactive"])
                    elif tool == "codex":
                        codex_spec = resolve_codex_args(
                            None, args_str if args_str else None
                        )
            ...
            if previews:
                ...
                lines.append(
                    f"    {DIM}{FG_GRAY}{truncate_ansi(preview_text, width - 4)}{RESET}"
                )
            else:
                # Show available fields description
                if tool == "claude":
                    lines.append(
                        f"    {DIM}{FG_GRAY}prompt, system, append, headless, args{RESET}"
                    )
                elif tool in RELEASED_BACKGROUND:
                    lines.append(
                        f"    {DIM}{FG_GRAY}prompt, system, headless, args{RESET}"
                    )
                else:
        ...
        result = self.render_section_fields(
            lines,
            claude_fields,
            self.state.claude_expanded,
            LaunchField.CLAUDE_SECTION,
            self.state.claude_cursor,
            width,
            FG_CLAUDE_ORANGE,
        )
        ...
        def is_field_modified(f):
            ...
            if not f.value:  # Empty
                ...
                return bool(
                    default.strip().strip("'\"")
                )  # Modified if default is non-empty
        ...
        if hcom_selected:
            selected_field_start_line = len(lines)
            hcom_action = (
                "\u2190 collapse" if self.state.hcom_expanded else "\u2192 expand"
            )
            ...
            lines.append(
                f"  {FG_CYAN}{BOLD}{expand_marker} HCOM{RESET}{FG_GRAY}{hcom_count}{RESET}"
            )
        ...
        result = self.render_section_fields(
            lines,
            hcom_fields,
            self.state.hcom_expanded,
            LaunchField.HCOM_SECTION,
            self.state.hcom_cursor,
            width,
            FG_CYAN,
        )
        if result is not None:
        ...
        custom_selected = (
            ...
            and self.state.custom_env_cursor == -1
        )
        ...
        if custom_selected:
            selected_field_start_line = len(lines)
            custom_action = (
                "\u2190 collapse" if self.state.custom_env_expanded else "\u2192 expand"
            )
            ...
            lines.append(
                f"  {FG_CUSTOM_ENV}{BOLD}{expand_marker} Custom Env{RESET}{FG_GRAY}{custom_count}{RESET}"
            )
        ...
        if not self.state.custom_env_expanded:
            if custom_set > 0:
                ...
                lines.append(
                    f"    {DIM}{FG_GRAY}arbitrary environment variables{RESET}"
                )
        ...
        result = self.render_section_fields(
            lines,
            custom_fields,
            self.state.custom_env_expanded,
            LaunchField.CUSTOM_ENV_SECTION,
            self.state.custom_env_cursor,
            width,
            FG_CUSTOM_ENV,
        )
        if result is not None:
        ...
        if editor_selected:
            ...
            if editor_available:
                lines.append(
                    f"  {FG_GRAY}\u2197 Open config in {editor_label_display}{RESET}"
                )
            else:
                lines.append(
                    f"  {FG_GRAY}\u2197 Open config in {editor_label_display} {FG_RED}(not found){RESET}"
                )
        ...
        if selected_field_start_line is not None:
            ...
            # Scroll down if selected field is below visible window
                ...
                selected_field_start_line >= self.state.launch_scroll_pos + form_height
            ):
                self.state.launch_scroll_pos = (
                    selected_field_start_line - form_height + 1
                )

            # Clamp scroll position
            self.state.launch_scroll_pos = max(
                0, min(self.state.launch_scroll_pos, max_scroll)
            )
        ...
        if len(lines) > form_height:
            ...
            visible_lines = lines[
                self.state.launch_scroll_pos : self.state.launch_scroll_pos
                + form_height
            ]
        ...
        if field_info:
            ...
            # HCOM_TIMEOUT: hidden from TUI (internal, headless/vanilla only)
                ...
                editor_color = FG_CYAN
                field_name = "Terminal"
                help_text = "default, terminal preset, or custom command with {script}"
            ...
            header = (
                f"{editor_color}{BOLD}{field_name}:{RESET} {FG_GRAY}{help_text}{RESET}"
            )
            lines.append(separator_line(width))
            lines.append(header)
            lines.append("")  # Blank line between header and input
            # Render editor with wrapping support
            editor_lines = render_text_input(
                field_value, cursor_pos, width, editor_content_rows, prefix=""
            )
    ...
    def handle_key(self, key: str):
        ...
        if key == "UP":
            ...
            if self.state.launch_field == LaunchField.CLAUDE_SECTION:
                ...
                if (
                    self.state.custom_env_cursor == -1
                    and not self.state.custom_env_expanded
                ):
        ...
        # LEFT/RIGHT: adjust count, cycle for cycle fields, cursor movement for text fields
            ...
            if self.state.launch_field == LaunchField.TOOL:
                options = list(self._tool_options)
                current = (
                    self.state.launch_tool
                    if self.state.launch_tool in options
                    else "claude"
                )
                idx = options.index(current)
                new_idx = (idx + 1) if key == "RIGHT" else (idx - 1)
                self.state.launch_tool = options[new_idx % len(options)]
                # Clear background state if new tool doesn't support it
                if self.state.launch_tool not in RELEASED_BACKGROUND:
                    self.state.launch_background = False
            # COUNT field: adjust by ±1
            elif self.state.launch_field == LaunchField.COUNT:
                try:
                    current = (
                        int(self.state.launch_count) if self.state.launch_count else 1
                    )
                    if key == "RIGHT":
                ...
                if field_info:
                    ...
                    if (
                        self.state.launch_field == LaunchField.CLAUDE_SECTION
                        and self.state.claude_cursor >= 0
                    ):
                        config_fields = self.build_claude_fields()
                        if self.state.claude_cursor < len(config_fields):
                            field_obj = config_fields[self.state.claude_cursor]
                    elif (
                        self.state.launch_field == LaunchField.HCOM_SECTION
                        and self.state.hcom_cursor >= 0
                    ):
                        config_fields = self.build_hcom_fields()
                        if self.state.hcom_cursor < len(config_fields):
                            field_obj = config_fields[self.state.hcom_cursor]

                    # Check if it's a cycle field
                    if field_obj and field_obj.field_type == "cycle":
                        # Special handling for HCOM_TERMINAL custom mode
                        if field_key == "HCOM_TERMINAL" and field_value not in (
                            field_obj.options or []
                        ):
                            # Currently in custom mode (value is a custom command)
                            # LEFT/RIGHT moves cursor in text, or cycles back to presets on empty
                            if not field_value:
                                # Empty custom, cycle back to presets
                                options = field_obj.options or []
                                new_idx = (
                                    len(options) - 2 if key == "LEFT" else 0
                                )  # Last preset or default
                                self.state.config_edit[field_key] = (
                                    options[new_idx] if options else "default"
                                )
                                self.state.config_field_cursors[field_key] = (
                                    len(options[new_idx]) if options else 7
                                )
                                ...
                                if key == "LEFT":
                                    ...
                                    new_cursor = text_input_move_right(
                                        field_value, cursor_pos
                                    )
                            ...
                            if options:
                                ...
                                if (
                                    field_key == "HCOM_TERMINAL"
                                    and new_value == "custom"
                                ):
                                    self.state.config_edit[field_key] = (
                                        ""  # Empty = custom edit mode
                                    )
                                    self.state.config_field_cursors[field_key] = 0
                                    self.tui.save_config_to_file()
                                else:
                                    self.state.config_edit[field_key] = new_value
                                    self.state.config_field_cursors[field_key] = len(
                                        new_value
                                    )
                                    self.tui.save_config_to_file()
                    else:
                        ...
                        if field_key == "prompt":
                            ...
                            if tool == "claude":
                                ...
                                self.state.launch_gemini_prompt_cursor = new_cursor
                            else:  # codex
                                self.state.launch_codex_prompt_cursor = new_cursor
                        elif field_key == "system_prompt":
                            self.state.launch_system_prompt_cursor = new_cursor
                        elif field_key == "append_system_prompt":
                            self.state.launch_append_system_prompt_cursor = new_cursor
                        elif field_key == "codex_system_prompt":
                            self.state.launch_codex_system_prompt_cursor = new_cursor
                        elif field_key == "gemini_system_prompt":
                            self.state.launch_gemini_system_prompt_cursor = new_cursor
                        else:
                            self.state.config_field_cursors[field_key] = new_cursor
                ...
                if self.state.claude_cursor < len(fields):
                    ...
                    if field.field_type == "checkbox":
                        if field.key == "background":
                            self.state.launch_background = (
                                not self.state.launch_background
                            )
                            self.tui.save_launch_state()
                    elif field.field_type == "cycle":
                        # Cycle to next option on ENTER (e.g., HCOM_CODEX_SANDBOX_MODE)
                        ...
                        if options:
                            ...
                            self.state.config_field_cursors[field.key] = len(new_value)
                            self.tui.save_config_to_file()
            elif (
                self.state.launch_field == LaunchField.HCOM_SECTION
                and self.state.hcom_cursor >= 0
            ...
            if self.state.launch_field == LaunchField.CLAUDE_SECTION:
                if self.state.claude_cursor >= 0:
                    ...
                    if self.state.claude_cursor < len(fields):
                        field = fields[self.state.claude_cursor]
                        if field.key == "prompt":
                            if self.state.launch_tool == "claude":
                                self.state.launch_prompt = ""
                                self.state.launch_prompt_cursor = 0
                                self.tui.save_launch_state()
                            elif self.state.launch_tool == "gemini":
                                self.state.launch_gemini_prompt = ""
                                self.state.launch_gemini_prompt_cursor = 0
                            else:  # codex
                                self.state.launch_codex_prompt = ""
                                self.state.launch_codex_prompt_cursor = 0
                            ...
                            self.state.launch_codex_system_prompt = ""
                            self.state.launch_codex_system_prompt_cursor = 0
                        elif field.key == "gemini_system_prompt":
                            self.state.launch_gemini_system_prompt = ""
                            self.state.launch_gemini_system_prompt_cursor = 0
                            ...
                            self.state.config_field_cursors["HCOM_CODEX_ARGS"] = 0
                            self.tui.save_config_to_file()
                        elif field.key == "gemini_args":
                            self.state.config_edit["HCOM_GEMINI_ARGS"] = ""
                            self.state.config_field_cursors["HCOM_GEMINI_ARGS"] = 0
                            self.tui.save_config_to_file()
                else:
        ...
        # SPACE and printable: insert at cursor
            ...
            if field_info:
                ...
                if field_key == "HCOM_TAG":
                    ...
                    if allowed_pattern:
                        test_value = (
                            field_value[:cursor_pos] + char + field_value[cursor_pos:]
                        )
    ...
    def get_command_preview(self) -> str:
        ...
        try:
            tool = (
                self.state.launch_tool
                if self.state.launch_tool in self._tool_options
                else "claude"
            )
            ...
            if tool == "claude":
                tool_prompt = self.state.launch_prompt
            elif tool == "gemini":
                tool_prompt = self.state.launch_gemini_prompt
            else:
                tool_prompt = self.state.launch_codex_prompt

            if tool_prompt:
                parts.append(
                    f"(prompt: {tool_prompt[:30]}{'...' if len(tool_prompt) > 30 else ''})"
                )
            else:
            ...
            if self.state.launch_append_system_prompt:
                tokens.extend(
                    ["--append-system-prompt", self.state.launch_append_system_prompt]
                )
        ...
        if (
            ...
            if self.state.claude_cursor < len(fields):
                ...
                if field.key == "prompt":
                    tool = self.state.launch_tool
                    if tool == "claude":
                        if self.state.launch_prompt_cursor > len(
                            self.state.launch_prompt
                        ):
                            self.state.launch_prompt_cursor = len(
                                self.state.launch_prompt
                            )
                        return (
                            "prompt",
                            self.state.launch_prompt,
                            self.state.launch_prompt_cursor,
                        )
                    elif tool == "gemini":
                        if self.state.launch_gemini_prompt_cursor > len(
                            self.state.launch_gemini_prompt
                        ):
                            self.state.launch_gemini_prompt_cursor = len(
                                self.state.launch_gemini_prompt
                            )
                        return (
                            "prompt",
                            self.state.launch_gemini_prompt,
                            self.state.launch_gemini_prompt_cursor,
                        )
                    else:  # codex
                        if self.state.launch_codex_prompt_cursor > len(
                            self.state.launch_codex_prompt
                        ):
                            self.state.launch_codex_prompt_cursor = len(
                                self.state.launch_codex_prompt
                            )
                        return (
                            "prompt",
                            self.state.launch_codex_prompt,
                            self.state.launch_codex_prompt_cursor,
                        )
                elif field.key == "system_prompt":
                    if self.state.launch_system_prompt_cursor > len(
                        self.state.launch_system_prompt
                    ):
                        self.state.launch_system_prompt_cursor = len(
                            self.state.launch_system_prompt
                        )
                    return (
                        "system_prompt",
                        self.state.launch_system_prompt,
                        self.state.launch_system_prompt_cursor,
                    )
                elif field.key == "append_system_prompt":
                    if self.state.launch_append_system_prompt_cursor > len(
                        self.state.launch_append_system_prompt
                    ):
                        self.state.launch_append_system_prompt_cursor = len(
                            self.state.launch_append_system_prompt
                        )
                    return (
                        "append_system_prompt",
                        self.state.launch_append_system_prompt,
                        self.state.launch_append_system_prompt_cursor,
                    )
                elif field.key == "codex_system_prompt":
                    if self.state.launch_codex_system_prompt_cursor > len(
                        self.state.launch_codex_system_prompt
                    ):
                        self.state.launch_codex_system_prompt_cursor = len(
                            self.state.launch_codex_system_prompt
                        )
                    return (
                        "codex_system_prompt",
                        self.state.launch_codex_system_prompt,
                        self.state.launch_codex_system_prompt_cursor,
                    )
                elif field.key == "HCOM_CODEX_SANDBOX_MODE":
                    # Cycle field - return current value with cursor at end
                    value = self.state.config_edit.get(
                    ...
                    if self.state.launch_gemini_system_prompt_cursor > len(
                        self.state.launch_gemini_system_prompt
                    ):
                        self.state.launch_gemini_system_prompt_cursor = len(
                            self.state.launch_gemini_system_prompt
                        )
                    return (
                        "gemini_system_prompt",
                        self.state.launch_gemini_system_prompt,
                        self.state.launch_gemini_system_prompt_cursor,
                    )
                elif field.key == "claude_args":
                    value = self.state.config_edit.get("HCOM_CLAUDE_ARGS", "")
                    cursor = self.state.config_field_cursors.get(
                        "HCOM_CLAUDE_ARGS", len(value)
                    )
                    ...
                    cursor = self.state.config_field_cursors.get(
                        "HCOM_CODEX_ARGS", len(value)
                    )
                    cursor = min(cursor, len(value))
                    self.state.config_field_cursors["HCOM_CODEX_ARGS"] = cursor
                    return ("HCOM_CODEX_ARGS", value, cursor)
                elif field.key == "gemini_args":
                    value = self.state.config_edit.get("HCOM_GEMINI_ARGS", "")
                    cursor = self.state.config_field_cursors.get(
                        "HCOM_GEMINI_ARGS", len(value)
                    )
                    cursor = min(cursor, len(value))
                    self.state.config_field_cursors["HCOM_GEMINI_ARGS"] = cursor
            ...
            self.state.launch_field == LaunchField.CUSTOM_ENV_SECTION
            and self.state.custom_env_cursor >= 0
        ):
            fields = self.build_custom_env_fields()
            if self.state.custom_env_cursor < len(fields):
                field = fields[self.state.custom_env_cursor]
    ...
    def update_field(self, field_key: str, new_value: str, new_cursor: int):
        ...
        if field_key == "prompt":
            ...
            if tool == "claude":
                ...
                self.state.launch_gemini_prompt = new_value
                self.state.launch_gemini_prompt_cursor = new_cursor
                self.tui.save_launch_state()
            else:  # codex
                self.state.launch_codex_prompt = new_value
                self.state.launch_codex_prompt_cursor = new_cursor
                self.tui.save_launch_state()
        elif field_key == "system_prompt":
            ...
            self.state.launch_codex_system_prompt = new_value
            self.state.launch_codex_system_prompt_cursor = new_cursor
            self.tui.save_launch_state()
        elif field_key == "gemini_system_prompt":
            self.state.launch_gemini_system_prompt = new_value
            self.state.launch_gemini_system_prompt_cursor = new_cursor
            self.tui.save_launch_state()
        elif field_key == "HCOM_CLAUDE_ARGS":
            self.state.config_edit[field_key] = new_value
            ...
            self.tui.save_config_to_file()
            self.tui.load_launch_state()
        elif field_key == "HCOM_CODEX_ARGS":
            self.state.config_edit[field_key] = new_value
            self.state.config_field_cursors[field_key] = new_cursor
            self.tui.save_config_to_file()
            self.tui.load_launch_state()
        elif field_key == "HCOM_GEMINI_ARGS":
            self.state.config_edit[field_key] = new_value
            self.state.config_field_cursors[field_key] = new_cursor
            self.tui.save_config_to_file()
            self.tui.load_launch_state()
    ...
    def build_claude_fields(self) -> List[Field]:
        ...
        if tool == "codex":
            ...
            current_sandbox_mode = self.state.config_edit.get(
                "HCOM_CODEX_SANDBOX_MODE", "workspace"
            )
    ...
    def build_hcom_fields(self) -> List[Field]:
        ...
        for key in expected_keys:
            ...
            if key == "HCOM_CODEX_SANDBOX_MODE":
                ...
                hint = sandbox_hints.get(value or "workspace", hint)
            fields.append(
                Field(
                    key,
                    display_name,
    ...
    def render_section_fields(
        ...
        if expanded or (
            self.state.launch_field == section_field and section_cursor >= 0
        ):
            visible_fields = fields if expanded else fields[:3]
            for i, field in enumerate(visible_fields):
                field_selected = (
                    self.state.launch_field == section_field and section_cursor == i
                )
                if field_selected:
                    selected_field_start_line = len(lines)
            ...
            if not expanded and len(fields) > 3:
                lines.append(
                    f"{FG_GRAY}    +{len(fields) - 3} more (enter to expand){RESET}"
                )

        return selected_field_start_line

    def render_field(
        self, field: Field, selected: bool, width: int, value_color: str | None = None
    ) -> str:
        ...
        if self.state.launch_tool == "claude" and field.key in (
            "prompt",
            "system_prompt",
            "append_system_prompt",
            "background",
        ...
        if field.field_type == "checkbox":
            ...
            if field.value:
                ...
                if field.key == "HCOM_RELAY_TOKEN" and field.value:
                    display_value = (
                        f"{field.value[:4]}***" if len(field.value) > 4 else "***"
                    )
                ...
                if runtime_reverts_to_default:
                    ...
                    value_str = (
                        f"{FG_WHITE}(default: {default}){RESET}"
                        if default
                        else f"{FG_WHITE}(empty){RESET}"
                    )
                else:
                    ...
                    if is_modified:
                        ...
                        value_str = (
                            f"{value_color}(empty) {FG_GRAY}default: {default}{RESET}"
                        )
                    else:
            ...
            if field.key == "HCOM_TERMINAL":
                ...
                if field.value and field.value not in preset_names:
                    ...
                    # Preset value
                    is_available = preset_avail.get(field.value, True)
                    ...
                    value_str = f"{FG_GRAY}(type custom command){RESET}"
            else:
                # Generic cycle field
                if field.value:
        ...
        if selected:
            ...
            return truncate_ansi(
                f"{indent}{FG_WHITE}{field.display_name}:{RESET} {value_str}", width
            )
    ...
    def get_footer(self) -> str:
        ...
        # Launch button
            ...
            return (
                f"{FG_GRAY}tab: switch  enter: install code CLI or set $EDITOR{RESET}"
            )
                ...
                if field.field_type == "checkbox":
                    ...
                    if field.key == "HCOM_TERMINAL":
                        ...
                        if value and value not in preset_names:
                            ...
                            return f"{FG_GRAY}tab: switch  type: command  ←→: back to presets  ctrl+r: reset config{RESET}"
    ...
    def do_launch(self):
        ...
        if self.state.validation_errors:
            ...
            self.tui.flash_error(
                f"Fix config errors before launching: {error_fields}", duration=15.0
            )
            return
        ...
        try:
            ...
            self.tui.flash(
                f"Launching {count} {tool} instance{'s' if count != 1 else ''}..."
            )
            ...
            if tool == "claude":
                ...
                if spec.errors:
                    self.tui.flash_error(
                        f"Invalid HCOM_CLAUDE_ARGS: {'; '.join(spec.errors)}"
                    )
                    return

                # Update spec with background and prompt
                background = self.state.launch_background if tool == "claude" else False
                spec = spec.update(
                ...
                result = unified_launch(
                    ...
                    codex_args,
                    tag=(self.state.config_edit.get("HCOM_TAG", "") or None),
                    prompt=self.state.launch_codex_prompt or None,
                    system_prompt=self.state.launch_codex_system_prompt or None,
                    background=headless,
                    run_here=False,  # TUI: always launch in new terminal
                )
            else:
                ...
                if config_args:
                    ...
                    if gemini_spec.errors:
                        self.tui.flash_error(
                            f"Invalid HCOM_GEMINI_ARGS: {'; '.join(gemini_spec.errors)}"
                        )
                        return
                ...
                result = unified_launch(
                    ...
                    gemini_args,
                    tag=(self.state.config_edit.get("HCOM_TAG", "") or None),
                    prompt=self.state.launch_gemini_prompt or None,
                    system_prompt=self.state.launch_gemini_system_prompt or None,
                    background=headless,
                    run_here=False,  # TUI: always launch in new terminal
                )
            ...
            if result and result.get("launched", 0) > 0:
                ...
                self.tui.flash(
                    f"Launched {result['launched']} instance{'s' if result['launched'] != 1 else ''}"
                )
